...
#[sqlx::test]
async fn subscribe_persists_the_new_subscriber(pool: Pool<Postgres>) {
    // Arrange
    let app = spawn_app(pool).await;
    let body = "name=le%20guin&email=ursula_le_guin%40gmail.com";
    Mock::given(path("/email"))
...
#[sqlx::test]
async fn subscribe_sends_a_confirmation_email_with_a_link(pool: Pool<Postgres>) {
    let app = spawn_app(pool).await;
    let body = "name=le%20guin&email=ursula_le_guin%40gmail.com";
    Mock::given(path("/email"))
        .and(method("POST"))
    ...
    let body = serde_html_form::from_bytes::<HashMap<String, String>>(&email_request.body).unwrap();
    ...
    // let html_link = get_link(body.get().as_str().unwrap());
    let text_link = get_link(body["TextBody"].as_str());
